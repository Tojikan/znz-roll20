...
let content = '<html> <h2>Abilities</h2>';


for (let a of abilities){
    content += `<h3>${a.label}</h3>`;
    content += '<ol>'
    for (let b of a.levels){
        content += `<li><strong>${b.label}</strong> - ${b.tip}</li>`
    }
    content += '</ol>'
}

content += '<br/><br/><br/><h2>Flaws</h2><ul>';


for (let f of flaws){
    content += `<li><strong>${f.label}</strong> - ${f.tip}</li>`;
}

content += '</ul></html>';


fs.writeFile('traitsdoc.html', content, function(){
    console.log('done!');
});
